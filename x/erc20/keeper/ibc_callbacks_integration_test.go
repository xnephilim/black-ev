...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert ablack", func() {
			...
			ablackInitialBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			// 1. Send ablack from Evmos to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.EvmosChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			ablackAfterBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			ablackIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AevmosIbcdenom)
			s.Require().Equal(amount, ablackIBCBalanceBefore.Amount.Int64())

			// 2. Send ablack IBC coins from Osmosis to Evmos
			...
			ablackIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AevmosIbcdenom)
			...
			ablackFinalBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
